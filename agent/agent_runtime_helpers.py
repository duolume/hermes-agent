...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        return _ra().handle_function_call(
            ...
            tool_ctx=build_tool_ctx(agent, tool_call_id, messages),
        )


def build_tool_ctx(agent, tool_call_id: Optional[str], messages: Optional[list]) -> dict:
    """Build the minimal context dict threaded into plugin tool handlers via
    ``registry.dispatch(... tool_ctx=...)``. Exposes:

    * ``session_id`` — the SQLite session_id this turn belongs to.
    * ``tool_call_id`` — the model-minted tool_use id, paired against
      state.db rows.
    * ``flush_session_db`` — a no-arg callable that force-persists the
      in-progress assistant ``tool_use`` turn so a gateway crash mid-tool
      doesn't lose the question itself to the next session resume.

    Used by the Arven durable-widget plugin (see apps/hermes-plugin/
    ask_user_question.py) and any other plugin that needs to "park, durably
    wait, and resume." Built in this helper so all three tool-dispatch
    paths (concurrent via invoke_tool, sequential at tool_executor.py:749/769,
    streaming at conversation_loop.py:4045) construct an identical ctx.
    """
    def _flush_db() -> None:
        try:
